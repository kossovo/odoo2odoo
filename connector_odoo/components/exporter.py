...
Exporters for distant Odoo.

In addition to its export job, an exporter has to:

* check in distant Odoo if the record has been updated more recently than the
...
class OdooBaseExporter(AbstractComponent):
    """ Base exporter for distant Odoo """
    ...
    def _should_import(self):
        ...
        distant_date = datetime.strptime(record['updated_at'],
                                         '%Y-%m-%d %H:%M:%S')
        return sync_date < distant_date
...
class OdooExporter(AbstractComponent):
    """ A common flow for the exports to distant Odoo """
    ...
    def _create(self, data):
        """ Create the distant Odoo record """
